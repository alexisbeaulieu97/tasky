# Spec: Task Import Capability

**Capability**: `task-import`
**Status**: Draft
**Package**: `tasky-tasks`
...
Defines the requirements for importing tasks from JSON files with comprehensive validation, error handling, and multiple merge strategies. The import capability enables backup restoration, data migration, and task template loading.
...
- Import function: `TaskImportExportService.import_tasks(file_path: Path, strategy: str, dry_run: bool) -> ImportResult`
- Located in: `packages/tasky-tasks/src/tasky_tasks/export.py`
- Strategies: "append" (default), "replace", "merge"
- Validation happens before ANY changes (atomic behavior)
- Error types: InvalidExportFormatError, IncompatibleVersionError, TaskValidationError
- Result format: ImportResult with (total_processed, created, updated, skipped, errors)
...
### Unit Tests
- Append strategy creates new tasks and re-keys duplicates
- Replace strategy deletes all first, then imports
- Merge strategy updates by ID, creates if new
- Invalid JSON raises InvalidExportFormatError
- Version mismatch raises IncompatibleVersionError
- Missing fields raise TaskValidationError
- Dry-run shows correct preview without changes
- Empty file handled gracefully
- All timestamp fields properly converted

### Integration Tests
- Full import workflow with real JSON file
- Each strategy produces correct result count
- Import summary message formatted correctly
- Large files (1000+ tasks) import successfully
- Existing task data not corrupted
- CLI error messages helpful for each error type

---

## Related Specifications

- `task-export`: Complementary export capability
- `task-import-strategies`: Detailed merge strategy documentation
- Relies on: `task-state-transitions` (for TaskStatus validation)
- Relies on: `automatic-timestamps` (for timestamp handling)
