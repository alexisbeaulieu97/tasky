# Specification Delta: Hierarchical Settings

**Change ID**: `add-hierarchical-configuration`
**Capability**: `hierarchical-settings`
**Type**: New Capability
...
- **Depends on**: None (uses pydantic-settings)
- **Depended on by**:
  - `global-configuration` (uses hierarchical-settings models and sources)
  - `project-configuration` (uses hierarchical-settings models and sources)
  - `logging-infrastructure` (modified to use settings)
...
- Existing code continues to work without settings
- Settings can be adopted incrementally in different packages
