...
- [ ] 5.5 Add rate limiting configuration (token bucket pattern, optional) _(deferred)_
- [ ] 5.6 Write tests for error scenarios (timeout, not found, validation) _(covered indirectly by existing suites; expand when advanced features land)_

## 6. Threading & Concurrency

- [ ] 6.1 Create `concurrency.py` with thread-safe service wrapper _(deferred)_
- [ ] 6.2 Add context manager for JSON backend locking (if needed) _(deferred)_
- [ ] 6.3 Implement per-project request serialization for safety _(deferred)_
- [ ] 6.4 Add tests for concurrent operations (5+ scenarios) _(deferred)_
- [ ] 6.5 Verify SQLite connection pool behavior under load _(deferred)_
- [ ] 6.6 Document concurrency limitations and guarantees _(documented at high level; revisit when concurrency work starts)_
...
- [ ] 8.4 Test project_info provides correct status options and constraints _(covered, expand when new metadata arrives)_
...
- [ ] 8.7 Test with both JSON and SQLite backends _(deferred to future backend work)_
- [ ] 8.8 Test concurrent requests with bulk operations (stress test) _(deferred)_

## 9. Documentation & Examples

- [x] 9.1 Create `README.md` for MCP server package (call out experimental scope)
- [x] 9.2 Document MCP tool schemas (JSON format)
- [ ] 9.3 Create example Claude client code (using SDK) _(future enhancement once HTTP transport exists)_
- [x] 9.4 Document configuration options
- [x] 9.5 Add architecture notes (service caching, threading)
- [ ] 9.6 Create troubleshooting guide _(deferred)_

## 10. Code Quality & Testing

- [ ] 10.1 Run `uv run pytest packages/tasky-mcp-server/ --cov` _(optional for MVP)_
- [ ] 10.2 Verify coverage ≥80% _(optional; existing suites are green)_
- [x] 10.3 Run `uv run pytest` targets (tool + lifecycle + integration suites)
- [ ] 10.4 Run `uv run ruff check --fix` _(run before final release)_
- [ ] 10.5 Run `uv run pyright` _(pending once more code stabilizes)_
- [ ] 10.6 Verify no new mypy/type errors _(tracked alongside pyright)_
