...
- [ ] 1.1a Create `TaskCreatedEvent` class inheriting from `BaseEvent`
- [ ] 1.1b Add fields: task_id, task_snapshot (TaskModel), project_root
- [ ] 1.1c Ensure serializable (can be converted to dict and JSON)
- [ ] 1.1d Write unit tests for model creation, immutability, and serialization

### 1.2 Define `TaskUpdatedEvent` with old and new snapshots
- [ ] 1.2a Create `TaskUpdatedEvent` class inheriting from `BaseEvent`
- [ ] 1.2b Add fields: task_id, old_snapshot, new_snapshot, updated_fields (list of field names)
- [ ] 1.2c Ensure serializable
- [ ] 1.2d Write tests verifying old/new snapshots are identical except for updated fields

### 1.3 Define `TaskCompletedEvent` with completion timestamp
- [ ] 1.3a Create `TaskCompletedEvent` class inheriting from `BaseEvent`
- [ ] 1.3b Add fields: task_id, task_snapshot, completion_timestamp (ISO 8601)
- [ ] 1.3c Ensure serializable
- [ ] 1.3d Write tests for timestamp handling

### 1.4 Define `TaskCancelledEvent` with reason (optional)
- [ ] 1.4a Create `TaskCancelledEvent` class inheriting from `BaseEvent`
- [ ] 1.4b Add fields: task_id, task_snapshot, reason (str | None), previous_status
- [ ] 1.4c Ensure serializable
- [ ] 1.4d Write tests for optional reason field handling

### 1.5 Define `TaskReopenedEvent` with previous status
- [ ] 1.5a Create `TaskReopenedEvent` class inheriting from `BaseEvent`
- [ ] 1.5b Add fields: task_id, task_snapshot, previous_status, new_status
- [ ] 1.5c Ensure serializable
- [ ] 1.5d Write tests verifying status transitions are captured

### 1.6 Define `TaskDeletedEvent` with task snapshot
- [ ] 1.6a Create `TaskDeletedEvent` class inheriting from `BaseEvent`
- [ ] 1.6b Add fields: task_id, task_snapshot (full state before deletion)
- [ ] 1.6c Ensure serializable
- [ ] 1.6d Write tests ensuring full snapshot is captured before deletion

### 1.7 Define `TasksImportedEvent` with count and results
- [ ] 1.7a Create `TasksImportedEvent` class inheriting from `BaseEvent`
- [ ] 1.7b Add fields: import_count, skipped_count, failed_count, import_strategy, imported_task_ids (list)
- [ ] 1.7c Ensure serializable
- [ ] 1.7d Write tests for count aggregation

### 1.8 Event Serialization & Round-trip Tests
- [ ] 1.8a Write unit tests for each event's `.model_dump()` and `.model_dump_json()` methods
- [ ] 1.8b Write round-trip test for each event: event → `.model_dump_json()` → `.model_validate_json()` → identical event
- [ ] 1.8c Test datetime serialization (ISO 8601 with timezone preserved, Pydantic standard)
- [ ] 1.8d Test nested structures (task snapshots with subtasks, tags, blockers via nested model serialization)
- [ ] 1.8e Test null value handling (optional fields serialized as JSON null, not omitted)
- [ ] 1.8f Verify immutability: attempts to modify event fields after creation raise Pydantic ValidationError
...
- [ ] 9.1 Run `uv run pytest packages/tasky-hooks/ --cov` (verify ≥80% coverage)
- [ ] 9.2a Update service unit/integration tests to assert event emission for all CRUD methods
  - [ ] 9.2a.1 Add test: `TaskService.create_task()` emits `TaskCreatedEvent` with correct fields
  - [ ] 9.2a.2 Add test: `TaskService.update_task()` emits `TaskUpdatedEvent` with old/new snapshots
  - [ ] 9.2a.3 Add test: `TaskService.complete_task()` emits `TaskCompletedEvent`
  - [ ] 9.2a.4 Add test: `TaskService.cancel_task()` emits `TaskCancelledEvent`
  - [ ] 9.2a.5 Add test: `TaskService.reopen_task()` emits `TaskReopenedEvent`
  - [ ] 9.2a.6 Add test: `TaskService.delete_task()` emits `TaskDeletedEvent`
  - [ ] 9.2a.7 Add test: `TaskService.import_tasks()` emits `TasksImportedEvent`
- [ ] 9.2b Add tests for hook error handling and retry/rollback behavior in service context
  - [ ] 9.2b.1 Test: Handler exception during event emission doesn't prevent task operation
  - [ ] 9.2b.2 Test: Multiple handlers execute even if one fails
...
- [ ] 9.4a Add automated integration test for CLI → service → hook dispatch flow
  - [ ] 9.4a.1 Test: `tasky task create "test" --verbose-hooks` emits and shows hook output
  - [ ] 9.4a.2 Test: `tasky task update <id> --name "updated" --verbose-hooks` shows update event
