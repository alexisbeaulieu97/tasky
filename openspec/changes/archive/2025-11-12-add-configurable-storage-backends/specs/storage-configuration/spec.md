# Capability: Storage Configuration

**Status**: Draft
**Capability ID**: `storage-configuration`
...
├── src/tasky_settings/
│   ├── __init__.py                 # Exports StorageSettings
│   ├── models.py                   # Contains StorageSettings class
│   └── ...
└── tests/
    ├── test_models.py              # StorageSettings tests
    └── ...
```
...
class StorageSettings(BaseModel):
    ...
    Attributes:
        backend: Backend type (e.g., "json", "sqlite", "postgres")
        path: Relative path from .tasky/ directory to storage location
    """

    backend: str = Field(default="json", min_length=1)
...
class AppSettings(BaseSettings):
    """Application settings with hierarchical sources."""

    # ... existing fields (logging, task_defaults, etc.) ...
...
- [ ] StorageSettings instantiates with default values
- [ ] StorageSettings accepts custom backend and path
- [ ] StorageSettings validates non-empty backend name
- [ ] AppSettings includes storage field
- [ ] Storage settings loaded from [storage] section in TOML config
- [ ] Storage settings respect hierarchical precedence
- [ ] Environment variables override project config for storage
- [ ] Defaults apply when no config exists
...
- Part of: `hierarchical-settings` (from `add-hierarchical-configuration`)
- Used by: `task-service-factory` (reads storage config to select backend)
- Related to: `backend-registry` (backend selection mechanism)
