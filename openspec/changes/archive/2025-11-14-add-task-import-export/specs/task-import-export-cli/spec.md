# Spec: Task Import/Export CLI Commands

**Capability**: `task-import-export-cli`
**Status**: Draft
**Package**: `tasky-cli`
**Layer**: Presentation
...
Defines the CLI commands and user interface for task import/export functionality. Includes command syntax, options, error handling, and user feedback.
...
### Export Command
- Function: `export_command(file_path: str)` in `packages/tasky-cli/src/tasky_cli/commands/tasks.py`
- Uses `TaskImportExportService.export_tasks(Path(file_path))`
- Creates parent directory with `path.parent.mkdir(parents=True, exist_ok=True)`
- Catches and formats exceptions with user-friendly messages

### Import Command
- Function: `import_command(file_path: str, strategy: str = "append", dry_run: bool = False)`
- Parameter: `file_path` as positional argument
- Option: `--strategy` / `-S` with choices ["append", "replace", "merge"]
- Flag: `--dry-run` boolean flag
- Uses `TaskImportExportService.import_tasks(Path(file_path), strategy, dry_run)`
- Validates strategy before calling service

### Error Handling
- `FileNotFoundError`: "✗ File not found: {path}"
- `InvalidExportFormatError`: "✗ Invalid file format: {message}"
- `IncompatibleVersionError`: "✗ Incompatible format version: {version}"
- `TaskValidationError`: "✗ Invalid task data: {message}"
- `ImportError`: "✗ Import failed: {message}"
- Generic exception: "✗ Unexpected error: {message}"

### Success Messages
- Export: `"✓ Exported {count} tasks to {path}"`
- Import append: `"✓ Import complete: {created} created, {updated} updated"`
- Import dry-run: `"[DRY RUN] Would import: {created} created, {updated} updated"`
...
### Unit Tests
- Export command creates file and shows success message
- Import command accepts all strategy options
- Import command accepts dry-run flag
- Invalid strategy rejected with helpful message
- File not found error handled gracefully
- Invalid JSON error handled gracefully
- Version incompatibility error handled gracefully

### Integration Tests
- Full export → import workflow works end-to-end
- All three strategies work via CLI
- Dry-run shows correct preview
- Error messages are helpful for all error types
- Large imports show correct summary

### Manual Testing
- `tasky task export --help` shows helpful information
- `tasky task import --help` shows helpful information
- `tasky task export backup.json` creates file successfully
- `tasky task import backup.json` imports with defaults
- `tasky task import backup.json --strategy replace` works correctly
- `tasky task import backup.json --dry-run` shows preview without changes
...
- `task-export`: Export functionality
- `task-import`: Import functionality
- `task-import-strategies`: Strategy behaviors
