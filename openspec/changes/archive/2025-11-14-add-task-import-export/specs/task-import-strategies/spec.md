# Spec: Task Import Merge Strategies

**Capability**: `task-import-strategies`
**Status**: Draft
**Package**: `tasky-tasks`
**Layer**: Domain
...
Defines the three distinct merge strategies for importing tasks and their exact behaviors, especially for handling task ID conflicts. Strategies provide different tradeoffs for various import scenarios.
...
- Strategy enum: `ImportStrategy.APPEND`, `ImportStrategy.REPLACE`, `ImportStrategy.MERGE`
- Each strategy is separate method: `_apply_append_strategy()`, `_apply_replace_strategy()`, `_apply_merge_strategy()`
- Located in: `packages/tasky-tasks/src/tasky_tasks/export.py`
- Return type: `ImportResult` with (created, updated, skipped counts)
- All strategies operate on validated TaskModel lists before database writes
- Strategies are atomic at validation step (all-or-nothing)
...
### Unit Tests
- Append: adds all tasks, re-keys duplicates, preserves timestamps
- Replace: deletes all, imports all, empty file handled
- Merge: updates by ID, creates if new, preserves unaffected tasks
- Strategy parameter validation and case-insensitivity
- Each strategy produces correct ImportResult counts

### Integration Tests
- Full import workflow with each strategy
- Large file imports with mixed duplicate/new tasks
- Existing tasks preserved correctly
- Updated timestamps consistent

---

## Related Specifications

- `task-import`: Import capability using these strategies
- Relies on: `task-state-transitions` (for task status validation)
- Relies on: `automatic-timestamps` (for timestamp updates)
