...
1. **Valid Transitions Only**: Enforce allowed transitions: `pending → completed|cancelled`, `completed|cancelled → pending`
2. **Explicit Intent**: Service methods clearly express business operations (`complete_task` vs raw status change)
3. **Clear Validation**: Attempting invalid transitions raises `InvalidStateTransitionError` with context
4. **Timestamp Integration**: State transitions automatically update `updated_at` via `mark_updated()`
5. **User-Friendly CLI**: Commands match user mental model (`complete`, `cancel`, `reopen`)
6. **Extensible**: Easy to add new statuses and transition rules in centralized location
...
1. **Transition Enforcement**: Invalid transitions (e.g., `cancelled → completed`) raise `InvalidStateTransitionError`
2. **Method Availability**: `complete_task()`, `cancel_task()`, `reopen_task()` methods work correctly
3. **Timestamp Updates**: All state transitions update `updated_at` timestamp
4. **CLI Commands**: `tasky task complete/cancel/reopen <id>` execute successfully
5. **Error Messages**: Invalid transition attempts show clear, actionable error messages
6. **Test Coverage**: ≥90% coverage for state machine logic and transition validation
7. **Documentation**: State diagram included in code comments and/or docs
