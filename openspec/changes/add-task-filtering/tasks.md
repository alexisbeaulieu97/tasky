...
- [x] **Task 1.1**: Extend `TaskRepository` protocol with `get_tasks_by_status(status: TaskStatus)` method
...
- [x] **Task 1.2**: Add convenience methods to `TaskService`
...
- [x] **Task 1.3**: Write unit tests for service filtering methods
...
- [x] **Task 2.1**: Implement `get_tasks_by_status()` in `JsonTaskRepository`
...
- [x] **Task 2.2**: Write integration tests for JSON filtering
...
- [x] **Task 3.1**: Add `--status` option to `task list` command
...
- [x] **Task 3.2**: Implement status validation and filtering logic
...
- [x] **Task 3.3**: Format filtered results consistently
...
- [x] **Task 4.1**: Add end-to-end CLI tests
...
- [x] **Task 4.2**: Run full test suite and fix issues
  - Run `uv run pytest` across all packages
  - Address any failures or regressions
  - Verify test coverage meets ≥80% target
  - **Validation**: All tests pass (207/207) - improved CLI validation to auto-derive valid statuses from enum, simplified error messages, and reorganized test files for consistency
...
- [x] **Task 5.1**: Manual smoke testing
  ...
  - **Validation**: All filtering scenarios work correctly (tested via end-to-end tests)
