# Spec: Task Filtering JSON Backend Implementation

**Capability**: `task-filtering-json-backend`  
**Status**: Draft  
...
Implements status-based task filtering in the JSON storage backend. Uses in-memory filtering with list comprehensions to match tasks by status.
...
- Add to: `packages/tasky-storage/src/tasky_storage/backends/json/repository.py`
- Implementation approach:
  ...
  ```
- Use existing `_load_document_optional()` and `_snapshot_to_task()` helpers
- Import `TaskStatus` from `tasky_tasks.models`
- Compare against `status.value` to match snapshot string representation
...
- Test with mixed task populations
...
**Test File**: `packages/tasky-storage/tests/test_json_repository.py`

**Test Cases**:
1. `test_get_tasks_by_status_returns_matching_tasks`
2. `test_get_tasks_by_status_returns_empty_when_no_matches`
3. `test_get_tasks_by_status_handles_missing_file`
4. `test_get_tasks_by_status_preserves_task_data`
5. `test_get_tasks_by_status_filters_all_statuses_correctly`
6. `test_get_tasks_by_status_performance` (optional)

---

## Related Specifications

- `task-filtering-protocol`: Protocol being implemented
- `task-filtering-service`: Service layer consumer
