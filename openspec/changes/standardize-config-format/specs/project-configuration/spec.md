...
### Requirement: Project configuration must be JSON format

**Reason**: JSON format is being replaced with TOML for consistency with global configuration and improved user experience.

**Migration**: Existing JSON files (`.tasky/config.json`) are automatically detected and converted to TOML format on first read with a clear warning message.
