...
- [x] 1.1 Define `ErrorResult` dataclass in `error_dispatcher.py`
...
- [x] 2.2 Update `_handle_task_validation_error` to return `ErrorResult`

- [x] 2.3 Update `_handle_invalid_transition` to return `ErrorResult`

- [x] 2.4 Update `_handle_invalid_export_format_error` to return `ErrorResult`

- [x] 2.5 Update `_handle_incompatible_version_error` to return `ErrorResult`

- [x] 2.6 Update `_handle_export_error` to return `ErrorResult`

- [x] 2.7 Update `_handle_import_error` to return `ErrorResult`

- [x] 2.8 Update `_handle_generic_task_domain_error` to return `ErrorResult`

- [x] 2.9 Update `_handle_storage_error` to return `ErrorResult`

- [x] 2.10 Update `_handle_project_not_found` to return `ErrorResult`

- [x] 2.11 Update `_handle_backend_not_registered` to return `ErrorResult`

- [x] 2.12 Update `_handle_pydantic_validation_error` to return `ErrorResult`

- [x] 2.13 Update `_handle_unexpected_error` to return `ErrorResult` (exit_code=2)

- [x] 2.14 Update `_format_error` helper to build and return `ErrorResult`

## 3. Update CLI Presentation Layer

- [x] 3.1 Update `with_task_error_handling` decorator in `tasks.py`
...
- [x] 3.2 Add `_format_error_for_cli(result: ErrorResult) -> str` helper
...
- [x] 4.1 Update `test_error_dispatcher.py` unit tests
...
- [x] 5.1 Update docstrings for `ErrorResult`, `ErrorHandler`, `ErrorDispatcher`
...
- [x] 5.3 Run type checker and verify no errors
...
- [x] 5.4 Run linter and fix any issues
  - `uv run ruff check packages/tasky-cli --fix`

- [x] 5.5 Verify CLI behavior unchanged (manual smoke test)
