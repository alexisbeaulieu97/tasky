...
## Module Focus
`tasky-cli` delivers the Typer-powered command-line interface.
...
- Add new CLI functionality by extending the modules in `tasky_cli.commands`.
...
## Project Layout
Implementation lives in `src/tasky_cli/`. Organise commands under `commands/` (e.g., `commands/tasks.py`). Shared presentation utilities (formatters, table printers) belong in `ui/`. Re-export the assembled Typer `app` from `src/tasky_cli/__init__.py` for reuse by other entry points.

## Development Commands
- `uv sync` prepares the environment with Typer and the declared dependencies.
- `uv run tasky` executes the Typer app for manual testing.
- `uv run tasky --help` is a quick check that command registration remains healthy.
